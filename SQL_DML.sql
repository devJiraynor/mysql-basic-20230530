...
# 변경사유발생일자의 데이터가 지정되어있는 데이터의 모든 컬럼을 검색
...
# OR, || : 좌항과 우항 중 하나로고 true일 때 true를 반환, 아니면 false를 반환
...
# 6. '용호_' : 뒤에 한 문자가 오고 '용호'라는 단어로 시작하고 길이가 3이면 true

# 위치_소재지에 '용호로'가 포함되어 있는 데이터의 전체 컬럼을 검색
SELECT * FROM namgu WHERE 위치_소재지 LIKE '%용호로%';

# 해체제거사업체명이 '(주)'로 시작하는 데이터의 전체 컬럼을 검색
